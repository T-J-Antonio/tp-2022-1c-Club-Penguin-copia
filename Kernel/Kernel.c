...
int main(void) {
	...
	void* _f_aux_escucha_consola(int *socket_kernel_escucha){
		escuchar_consola(socket_kernel_escucha, cola_procesos_nuevos, config);
		return NULL;
	}
	...
	void* pcb_serializado = serializar_header((pcb*)cola_procesos_nuevos);
...
void* escuchar_consola(int socket_kernel_escucha, void* cola_procesos_nuevos, t_config* config){
	while(1){//desde aca
